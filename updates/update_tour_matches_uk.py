...
功能：从 tennis-data.co.uk 下载比赛原始数据，将球员名称缩写映射为全名，并输出清洗后的比赛数据。

输入文件：
- ./tennis_{label}/{label}_players_active_rank.csv : 活跃球员排名表（必需，来源于tennisabstract）
  格式：包含 name 列（球员全名）
  例如：./tennis_wta/wta_players_active_rank.csv
- ./tennis_{label}/{label}_manual_mapping.csv : 手动映射表（可选）
  格式：包含 abbreviation, full_name 两列
- ./tennis_{label}/{label}_players.csv : 历史球员表（可选，use_history=True 时使用）
  格式：包含 name 或 name_first/name_last 列

输出文件：
1. ./scrape/{tour}_matches_{year}.xlsx : 从 tennis-data.co.uk 下载的原始数据
2. ./tennis_{tour}/{tour}_matches_{year}.csv : 清洗后的比赛数据（主要输出）
   - 球员名称已从缩写（如 "Djokovic N."）映射为全名
   - 轮次已标准化为 R128/R64/R32/R16/QF/SF/F/RR
   - 比分已格式化为统一格式
3. ./output/{tour}_matches_{year}_review.csv : 需人工审核的记录（仅当存在匹配失败时生成）
   - 包含未匹配成功的球员缩写及候选全名

运行方式：
    python update_tour_matches_uk.py

注意：需要确保 ./tennis_wta/ 或 ./tennis_atp/ 目录下存在对应的球员数据文件
"""
import sys, os
...
def download_uk_data(tour, year):
    
    # 要下载的URL
    if tour == 'wta':
        url = "http://tennis-data.co.uk/2026w/2026.xlsx"
    if tour == 'atp':
        url = "http://tennis-data.co.uk/2026/2026.xlsx"

    # 保存的文件名
    filename = f"./scrape/{tour}_matches_2026.xlsx"
...
def get_uk_data(uk_data_dir, tour='wta', year=2026):
    ...
    df['tourney_date'] = pd.to_datetime(df['Date'], errors='coerce').apply(
...
def map_uk_data_name(uk_data_dir, tour, year, use_history=False):
    ...
    review_path = f'./output/{tour}_matches_{year}_review.csv'
    if len(needs_review) > 0:
        needs_review[review_cols].to_csv(review_path, index=False)
        print(f"\n注意: 有 {len(needs_review)} 条记录需要人工审核，已保存至 {review_path}")
        return
    #########################################################################
    # 输出清洗后的比赛数据（用全名替换缩写）
    #########################################################################
    ...
    clean_path = f'./tennis_{tour}/{tour}_matches_{year}.csv'
...
if __name__ == "__main__":
    download_uk_data('wta', 2026)
    download_uk_data('atp', 2026) # 会临时生成scrape文件夹，但github action不会更新这部分
    ...
    # 删除临时文件夹./scrape和其所有文件
    if os.path.exists('./scrape'):
        for f in os.listdir('./scrape'):
            os.remove(os.path.join('./scrape', f))
        os.rmdir('./scrape')