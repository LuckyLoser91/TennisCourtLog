...
def process_tour(tour):
    ...
    for player in champions:
        ...
        # 获取该球员所有比赛记录（含 walkover，用于计算 best round）
        player_matches = gs[
        ...
        for age in range(MIN_AGE, MAX_AGE + 1):
            ...
            if not year_matches.empty:
                ...
                for slam in ["AO", "RG", "WIM", "USO"]:
                    ...
                    # 累计胜负数（排除 walkover）
                    valid_matches = slam_matches[slam_matches["score"] != "W/O"]
                    entry["W"] += len(valid_matches[valid_matches["winner_name"] == player])
                    entry["L"] += len(valid_matches[valid_matches["loser_name"] == player])
