...
输入: tennis_atp/atp_gs_matches.csv
...
CHAMPION_RANK = 8  # 夺冠对应的等级（高于输掉决赛的 F=7）
RANK_LABEL = {1: "R128", 2: "R64", 3: "R32", 4: "R16", 5: "QF", 6: "SF", 7: "F", 8: "W"}
...
def calc_age(birth_date, event_date):
    """
    计算年龄（年），保留一位小数。
    birth_date: datetime 对象或字符串（任意格式，pd.to_datetime 可识别）
    event_date: 字符串，支持 YYYY/MM/DD 和 YYYY-MM-DD 两种格式
    """
    try:
...
def build_match_records(gs, player, player_info):
    ...
    返回列表，每个元素为 dict，包含：
        ...
        opponent_rank  : 对手的实时排名（int），无排名数据时为 None
    """
    ...
    for _, row in wins.iterrows():
        ...
        # 胜场：对手是 loser，取 loser_rank
        opp_rank_raw = row.get("loser_rank")
        opponent_rank = int(opp_rank_raw) if pd.notna(opp_rank_raw) else None
    ...
    for _, row in losses.iterrows():
        ...
        # 负场：对手是 winner，取 winner_rank
        opp_rank_raw = row.get("winner_rank")
        opponent_rank = int(opp_rank_raw) if pd.notna(opp_rank_raw) else None
...
def accumulate_stats(records, max_age):
    ...
    新增字段：
        h2h_top8_W              : 截止该年龄，对阵排名 ≤8 对手的累计胜场
        h2h_top8_L              : 截止该年龄，对阵排名 ≤8 对手的累计负场
        h2h_top8_ranked_matches : 截止该年龄，对手有排名记录的总场次（用于说明数据覆盖范围）
    """
    cumulative = {
        "W"                     : 0,
        "L"                     : 0,
        ...
        "h2h_top8_W"            : 0,
        "h2h_top8_L"            : 0,
        "h2h_top8_ranked_matches": 0,   # 对手有排名的场次总数
        # ── 新增：各大满贯历史最佳战绩（不限于夺冠）──────────
        "best_by_slam"          : {slam: {"rank": 0, "count": 0} for slam in ["AO", "RG", "WIM", "USO"]},
    }

    def update_best_by_slam(slam, new_rank):
        """更新某项大满贯的历史最佳战绩等级及达到该等级的次数"""
        best = cumulative["best_by_slam"].get(slam)
        if best is None:
            return
        if new_rank > best["rank"]:
            best["rank"] = new_rank
            best["count"] = 1
        elif new_rank == best["rank"]:
            best["count"] += 1
        # new_rank < best["rank"] 时忽略（已有更好战绩）
    ...
    for age in range(MIN_AGE, max_age + 1):
        # 累加所有 age_float <= age 且尚未累加的记录
        while (record_idx < total_records
               and records[record_idx]["age_float"] is not None
               and records[record_idx]["age_float"] <= age):

            rec      = records[record_idx]
            opp_rank = rec.get("opponent_rank")
            ...
            if rec["is_win"]:
                ...
                if rec["round"] == 'F' and rec["slam_key"]:
                    ...
                    # 夺冠 = 该项大满贯历史最佳战绩等级
                    update_best_by_slam(slam, CHAMPION_RANK)
            else:
                cumulative["L"] += 1
                surf = rec["surface"]
                if surf in cumulative["loss_by_surface"]:
                    cumulative["loss_by_surface"][surf] += 1
                # 输球即为该年该项大满贯的出局轮次（正赛单败淘汰，一年最多一次）
                if rec["slam_key"]:
                    r = ROUND_RANK.get(rec["round"])
                    if r is not None:
                        update_best_by_slam(rec["slam_key"], r)

            # ── Top 8 H2H 累加 ────────────────────────────────
            if opp_rank is not None:
                cumulative["h2h_top8_ranked_matches"] += 1
                if opp_rank <= 8:
                    if rec["is_win"]:
                        cumulative["h2h_top8_W"] += 1
                    else:
                        cumulative["h2h_top8_L"] += 1
            # ─────────────────────────────────────────────────
        ...
        # ── 各大满贯历史最佳战绩（用于 4 个明细子列）──────────
        slam_breakdown = {}
        for slam in ["AO", "RG", "WIM", "USO"]:
            best = cumulative["best_by_slam"][slam]
            rank, cnt = best["rank"], best["count"]
            if rank == 0:
                slam_breakdown[slam] = {"display": "—", "rank": 0, "count": 0}
            else:
                slam_breakdown[slam] = {
                    "display": f"{RANK_LABEL[rank]}×{cnt}",
                    "rank": rank,
                    "count": cnt,
                }
        ...
        age_entries.append({
            ...
            "win_g"     : win_g,
            # ── 新增：Top 8 H2H ──────────────────────────────
            "h2h_top8_W"            : cumulative["h2h_top8_W"],
            "h2h_top8_L"            : cumulative["h2h_top8_L"],
            "h2h_top8_ranked_matches": cumulative["h2h_top8_ranked_matches"],
        })
...
def process_tour(tour):
    ...
    for _, row in champ_players_df.iterrows():
        ...
        height = row['height'] if 'height' in row and pd.notna(row['height']) else None
        player_info[name] = {'dob': dob, 'ioc': ioc, 'birth_year': birth_year, 'height': height}
    ...
    for player, first_year in champions.items():
        ...
        player_data = {
            ...
            "slam_breakdown": career.get("slam_breakdown"),
            "W"          : career.get("W"),
            "L"          : career.get("L"),
            "agg"        : career.get("agg"),
            ...
            "ioc"        : info.get("ioc"),
            "active": player in (ACTIVE_WTA if tour == "wta" else ACTIVE_ATP),
            "birth_year" : info.get("birth_year"),
            "dob"        : info.get('dob').strftime('%Y-%m-%d') if info.get('dob') else None,
            "height"     : info.get('height'),
            # ── 新增：Top 8 H2H 顶层字段（职业生涯总计）────────
            "h2h_top8_W"            : career.get("h2h_top8_W"),
            "h2h_top8_L"            : career.get("h2h_top8_L"),
            "h2h_top8_ranked_matches": career.get("h2h_top8_ranked_matches"),
            # ──────────────────────────────────────────────────
