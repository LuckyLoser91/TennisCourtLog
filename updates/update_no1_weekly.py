...
1. 更新球员的生涯胜场/负场/胜率（总体、硬地、红土、草地）
   —— 默认更新 JSON 中的全部球员（临时需求，见 UPDATE_ALL_PLAYERS 开关）
...
# ⚠️ 临时开关：
#   True  = 更新 JSON 中「所有球员」（当前需求，用于补齐胜场/负场字段）
#   False = 恢复原逻辑，只更新 ACTIVE_PLAYERS 名单中的现役球员
UPDATE_ALL_PLAYERS = False

# 现役球员名单（UPDATE_ALL_PLAYERS=False 时生效）
ACTIVE_PLAYERS = {
    # WTA
    "Victoria Azarenka",
    "Karolina Pliskova",
...
# ================================================================
# ── 2. 读取 JSON，并确定需要统计的球员集合 ──────────────────────
# ================================================================
with open(JSON_PATH, encoding="utf-8") as f:
    data = json.load(f)

def json_players(data):
    """依次产出 JSON 中 wta / atp 两个列表里的球员对象"""
    for section in ("wta", "atp"):
        for p in data.get(section, []):
            yield p

if UPDATE_ALL_PLAYERS:
    players_lower = {p["player"].lower() for p in json_players(data)}
    print(f"更新模式：JSON 中全部球员（{len(players_lower)} 人）")
else:
    players_lower = {p.lower() for p in ACTIVE_PLAYERS}
    print(f"更新模式：仅 ACTIVE_PLAYERS 现役球员（{len(players_lower)} 人）")

# ================================================================
# ── 3. 统计比赛数据（WTA + ATP）────────────────────────────────
# ================================================================
def read_matches(matches_glob, target_lower):
    """读取比赛文件，只统计 target_lower 中的球员，
    返回 {name_lower: {"overall": {"w":,"l":}, "hard": {...}, "clay": {...}, "grass": {...}}}"""
...
print("\n读取 WTA 比赛数据...")
wta_stats = read_matches(WTA_MATCHES, players_lower)
print("读取 ATP 比赛数据...")
atp_stats = read_matches(ATP_MATCHES, players_lower)

# 合并统计（同一球员不会同时出现在 WTA 和 ATP，直接合并字典即可）
...
# ================================================================
# ── 4. 更新 JSON 中每个球员的胜场 / 负场 / 胜率 ────────────────
# ================================================================
print("\n更新 JSON 胜场/负场/胜率...")

def update_winrate(player_obj, stats):
    """写入 wins / losses / win_rate 字段。
    找不到比赛数据的球员也会被写入 0 场，保证字段齐全。
    返回 True 表示确实找到了比赛数据。"""
    ...
    player_obj["wins_overall"]   = ov["w"]
    player_obj["losses_overall"] = ov["l"]
    player_obj["win_rate_overall"] = win_rate(ov["w"], ov["l"])

    # 各场地
    for surf in SURFACES:
        if not isinstance(player_obj.get(surf), dict):
            player_obj[surf] = {}
        d = s.get(surf, {"w": 0, "l": 0})
        player_obj[surf]["wins"]     = d["w"]
        player_obj[surf]["losses"]   = d["l"]
        player_obj[surf]["win_rate"] = win_rate(d["w"], d["l"])
...
for p in json_players(data):
    if p["player"].lower() not in players_lower:
        continue
    if update_winrate(p, all_stats):
        updated_count += 1
        print(f"  ✓ {p['player']}: "
              f"总体 {p['wins_overall']}-{p['losses_overall']} ({p['win_rate_overall']}%) | "
              f"硬地 {p['hard']['wins']}-{p['hard']['losses']} ({p['hard']['win_rate']}%) | "
              f"红土 {p['clay']['wins']}-{p['clay']['losses']} ({p['clay']['win_rate']}%) | "
              f"草地 {p['grass']['wins']}-{p['grass']['losses']} ({p['grass']['win_rate']}%)")
    else:
        missing_players.append(p["player"])
        print(f"  ⚠️ {p['player']}: 未找到比赛数据（已写入 0-0）")

print(f"\n共更新 {updated_count} 名球员的胜场/负场/胜率。")
if missing_players:
    print(f"⚠️ 以下 {len(missing_players)} 名球员未匹配到比赛数据，请检查姓名是否与 CSV 一致：")
