...
# 罗马战绩缓存文件路径
ROME_STATS_CACHE_PATH = "api_folder/data/rome_2026/rome_stats_cache.json"
...
def load_rome_stats_cache() -> Dict[str, Dict]:
    """
    加载罗马战绩缓存文件
    """
    if os.path.exists(ROME_STATS_CACHE_PATH):
        with open(ROME_STATS_CACHE_PATH, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {}

def save_rome_stats_cache(cache: Dict[str, Dict]):
    """
    保存罗马战绩缓存文件
    """
    with open(ROME_STATS_CACHE_PATH, 'w', encoding='utf-8') as f:
        json.dump(cache, f, indent=2, ensure_ascii=False)
    print(f"已更新罗马战绩缓存，共 {len(cache)} 名球员")

def find_rome_stats(name: str, stats_map: Dict, not_found: List, ranking: str) -> Dict:
    """查找球员的 Rome 统计数据"""
    if name in ['Bye', 'TBD']:
        return {'best_round': '', 'W': '', 'L': '', 'winrate': '', 'titles': ''}

    if name in stats_map:
        rome_data = stats_map[name]
        return {
            'best_round': rome_data.get('best_round', '') or '',
            'W': rome_data.get('W', ''),
            'L': rome_data.get('L', ''),
            'winrate': rome_data.get('winrate', ''),
            'titles': rome_data.get('titles', '')
        }
    else:
...
def get_rome_stats_for_players(
    ...
    """
    从 tennis_wta 文件夹的历年 CSV 中提取指定球员在 Rome 赛事的历史统计数据。
...
def enhance_draw():
    ...
    # 使用缓存机制获取 Rome 历史统计
    stats_map = get_rome_stats_for_players(
    ...
    print(f"成功获取 {len(stats_map)} 名球员的 Rome 历史数据")

    current_round = main_draw.get('currentRound', 1)
    not_found = []  # 未找到 Rome 历史数据的球员
    ...
    # 保存 JSON
    json_path = "./output/draw_rome_2026_with_history_stats.json"
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    # 打印未找到 Rome 历史数据的球员（仅打印有排名的）
    if len(not_found) > 0:
        print("\n以下球员未找到 Rome 赛事历史数据：")
